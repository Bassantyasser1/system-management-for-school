...
class Product
{
    ...
    public function __construct(int $Id = null,string $Name = null,float $Price = null,string $Type = null) {
       ...
       $this->File = new FileManger("product");
    }
    ...
    public function Update()
    {
        ...
        $OldProduct->FromStringToObject($Line);
        if($this->Name=="") $this->setName($OldProduct->getName());
        if($this->Price == 0) $this->setPrice($OldProduct->getPrice());
        if($this->Type == "") $this->setType($OldProduct->getType());
        $this->File->UpdateRecord($this->ToString(),$OldProduct->ToString());
        return 1;
    }
    ...
    public function Search()
    {
        ...
        for ($i=0; $i < count($List) - 1; $i++) { 
            $Product = new Product();
            $Product->FromStringToObject($List[$i]);
            if($this->Id!=0)
            {
